...
export const createOrderFromCart = async (userId) => {
  ...
  // permanently consuming the coupon (incrementing Coupon.usedCount and
  // writing a CouponRedemption record) is deferred to
  // paymentService.verifyPayment's success path, which calls
  // couponService.redeemCoupon only once Razorpay's signature has been
  // verified and the order is actually being confirmed.
